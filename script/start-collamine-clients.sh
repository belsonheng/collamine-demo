...
if [ -f $FILE ]; then
   ...
   ssh cm07 "nohup ruby /home/belson/gems/collamine/test.rb 2>&1 >> /tmp/ruby.log &"
   #screen -S ruby -d -m ruby ~/gems/collamine/test.rb
   echo "Start Python Crawler"
   ssh cm06 "nohup bash /home/belson/git/collamine-client-python/scrapybot/run.sh 2>&1 >> /tmp/python.log &"
